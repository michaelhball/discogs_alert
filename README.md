...
* `--log-file=<path>` — also append logs to a file, rotated by size (`runtime.log_max_bytes`, default 10 MiB × `runtime.log_backup_count`, default 5). stderr still gets everything unless `runtime.log_stderr = false`.
